...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # print('cha', sum(dist_ap)-sum(dist_an))
...
class DirectionOrientedTripletLoss(nn.Module):
    """Triplet loss with hard positive/negative mining.
    ...
    edited by liuyw: add consideration for direction
    """
    def __init__(self, margin=0.3, mutual_flag = False):
        super(DirectionOrientedTripletLoss, self).__init__()
        self.margin = margin
        self.ranking_loss = nn.MarginRankingLoss(margin=margin)
        self.mutual = mutual_flag

    def forward(self, inputs, targets, directions):
        """
        Args:
            inputs: feature matrix with shape (batch_size, feat_dim)
            targets: ground truth labels with shape (num_classes)
        ...
        # For each anchor, find the hardest positive and negative
        mask = targets.expand(n, n).eq(targets.expand(n, n).t())
        mask_direction = directions.expand(n, n).eq(directions.expand(n, n).t())
        #print(mask)
        #print(mask_direction)
        #print(mask[0] & mask_direction[0])
        dist_ap, dist_an = [], []
        for i in range(n):
            aps = dist[i][(mask[i]) & mask_direction[i]]
            ans = dist[i][(mask[i]==0) & mask_direction[i]]
            if aps.shape[0] == 0 or ans.shape[0] == 0:
                continue
            dist_ap.append(aps.max().unsqueeze(0))
            dist_an.append(ans.min().unsqueeze(0))
        #print(dist_ap, dist_an)

        if len(dist_ap) == 0:
            return torch.FloatTensor([0]).type(dist.type())
